...
> 题库声明：内置题库包含 800 道依据公开考纲编写的原创学习题，以及用户提供的 277 道答案型模拟题。导入题仅供个人学习，请确保拥有相应使用权限；本项目不复制或分发 Boson、Sybex/OSG 等商业题库，也不将任何题目标注为 CISSP 真题。
...
`public/data/questions.json` 包含 1,077 道练习：800 道原创练习（八域各 100 道），以及用户提供的 `CISSP2508模拟题_含答案.csv` 中 277 道答案型模拟题。导入题保留原题干、选项和答案，知识域通过关键词规则自动归类；由于源文件未附解析，系统会明确提示并允许使用 AI 深度解析进一步核对。
...
该命令依次执行 ESLint、严格 TypeScript 检查、单元测试、1,077 题题库校验和生产静态构建。人工浏览器验收记录见 [SELF_CHECK.md](./SELF_CHECK.md)。
