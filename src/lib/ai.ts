...
只输出 JSON，不使用 Markdown。结构必须为：{"type":"single|multiple","tags":["标签"],"stem":"题干","options":[{"id":"A","text":"..."},{"id":"B","text":"..."},{"id":"C","text":"..."},{"id":"D","text":"..."}],"correctAnswers":["A"],"explanation":{"logic":"核心作答逻辑","optionAnalysis":{"A":"逐项说明","B":"...","C":"...","D":"..."},"knowledgePoint":"考点定位","plainLanguage":"通俗企业场景解读"}}`;
  const response = await fetch(`${cleanBaseUrl(settings.baseUrl)}/chat/completions`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${settings.apiKey}` },
    body: JSON.stringify({ model: settings.model, temperature: 0.7, response_format: { type: "json_object" }, messages: [{ role: "system", content: "你只输出有效 JSON。" }, { role: "user", content: prompt }] }),
  });
  if (!response.ok) {
    const body = await response.text().catch(() => "");
    throw new Error(`AI 接口返回 ${response.status}${body ? `：${body.slice(0, 120)}` : ""}`);
  }
  const payload = await response.json() as { choices?: Array<{ message?: { content?: string } }> };
  const content = payload.choices?.[0]?.message?.content;
  if (!content) throw new Error("AI 没有返回题目内容");
  const raw = JSON.parse(content.replace(/^```json\s*|\s*```$/g, "")) as Record<string, unknown>;
...
export async function explainQuestion(settings: AISettings, question: Question, selectedAnswers: string[]): Promise<Explanation> {
  ...
  if (!response.ok) throw new Error(`AI 解析失败（HTTP ${response.status}），已保留内置解析`);
  const payload = await response.json() as { choices?: Array<{ message?: { content?: string } }> };
  const content = payload.choices?.[0]?.message?.content;
  if (!content) throw new Error("AI 没有返回解析，已保留内置解析");
  return explanationSchema.parse(JSON.parse(content.replace(/^```json\s*|\s*```$/g, "")));
}
