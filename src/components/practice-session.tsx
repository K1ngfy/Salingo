...
export function PracticeSession({ bankId, domainId, sectionId, reviewMode = false, questionIds, sessionMode = "practice" }: {
  ...
  return <div className="mx-auto max-w-3xl">
    ...
    <AnimatePresence>{checked && <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className={cn("mt-7 rounded-[1.6rem] p-5 sm:p-6", correct ? "bg-[var(--c-eefbdc)]" : "bg-[var(--c-fff0f0)]")}>
      <div className="flex items-center gap-3">{correct ? <CheckCircle size={30} weight="fill" className="text-[var(--c-58a700)]" /> : <XCircle size={30} weight="fill" className="text-[var(--c-e33d3d)]" />}<h2 className="text-xl font-black">{correct ? "判断正确" : `正确答案：${responseLabel(correctResponse(question))}`}</h2></div>
      <div className="mt-5 grid gap-4 text-sm leading-6 sm:grid-cols-2"><div className="rounded-2xl bg-[var(--surface)]/75 p-4 sm:col-span-2"><h3 className="flex items-center gap-2 font-black"><Lightbulb size={19} weight="duotone" />{aiExplanation ? "AI 核心作答逻辑" : "原始解析"}</h3><p className="mt-2 font-semibold text-[var(--c-666)]">{aiExplanation ? shownExplanation.logic : content.primary.explanation}</p>{content.secondary && !aiExplanation && <p className="mt-3 border-t border-[var(--c-deded8)] pt-3 font-semibold text-[var(--c-778086)]">{content.secondary.explanation}</p>}</div><div className="rounded-2xl bg-[var(--surface)]/75 p-4"><h3 className="flex items-center gap-2 font-black"><MapPin size={19} weight="duotone" />考点定位</h3><p className="mt-2 font-semibold text-[var(--c-666)]">{shownExplanation.knowledgePoint}</p></div><div className="rounded-2xl bg-[var(--surface)]/75 p-4"><h3 className="font-black">通俗解读</h3><p className="mt-2 font-semibold text-[var(--c-666)]">{shownExplanation.plainLanguage}</p></div>{hasOptionAnalysis && question.type !== "matching" && <div className="rounded-2xl bg-[var(--surface)]/75 p-4 sm:col-span-2"><h3 className="flex items-center gap-2 font-black"><Shuffle size={19} weight="duotone" />逐项排错</h3><ul className="mt-2 grid gap-2 sm:grid-cols-2">{question.options.map((option) => shownExplanation.optionAnalysis[option.id] && <li key={option.id} className="font-semibold text-[var(--c-666)]"><strong>{option.id}.</strong> {shownExplanation.optionAnalysis[option.id]}</li>)}</ul></div>}</div>
      {!correct && <div className="mt-4 rounded-xl bg-[var(--surface)]/70 p-3"><p className="text-xs font-black text-[var(--c-8a6a6a)]">这次错误主要属于</p><div className="mt-2 flex flex-wrap gap-2">{(["概念盲区", "审题失误", "混淆考点"] as MistakeType[]).map((type) => <button key={type} type="button" onClick={() => { setMistakeType(type); if (currentReview) { const nextReview = { ...currentReview, mistakeType: type }; setCurrentReview(nextReview); void upsertReview(nextReview); } }} className={cn("rounded-lg px-3 py-1.5 text-xs font-black", mistakeType === type ? "bg-[var(--c-ff4b4b)] text-white" : "bg-[var(--c-f3f3ef)] text-[var(--c-777)]")}>{type}</button>)}</div></div>}
      <div className="mt-4 flex flex-wrap items-center gap-3"><Button variant="secondary" size="sm" onClick={requestAIExplanation} disabled={aiLoading}><Sparkle size={17} weight="fill" />{aiLoading ? "AI 正在快速解析…" : aiExplanation ? "重新生成 AI 解析" : "AI 快速解析"}</Button>{aiError && <span className="text-xs font-bold text-[var(--c-c63838)]">{aiError}</span>}</div>
    </motion.section>}</AnimatePresence>
    {!checked && submitError && <p role="alert" className="mt-4 rounded-xl bg-[var(--c-fff0f0)] p-3 text-sm font-bold text-[var(--c-c63838)]">{submitError}</p>}
