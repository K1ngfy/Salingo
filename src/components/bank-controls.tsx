...
export function BankSelector({ value, onChange, allowAll = false }: {
  ...
  return <div className="w-full max-w-full sm:w-auto">
    <label className="flex w-full max-w-full items-center gap-2 overflow-hidden rounded-xl bg-white px-3 ring-2 ring-[#e7e7e1]">
      <Books size={19} weight="duotone" className="text-[#9b6b43]" />
    ...
    {error && <p className="mt-1 text-xs font-bold text-[#c63838]">{error}</p>}
  </div>;
}
...
export function ContentLanguageToggle() {
  ...
  return <div className="flex items-center gap-1 rounded-xl bg-[#f1f1ed] p-1" aria-label="题目语言">
    <Translate size={18} className="mx-1 text-[#777]" />
    {(["zh", "en", "bilingual"] as const).map((value) => <button key={value} type="button" onClick={() => setLanguage(value)} className={`rounded-lg px-2.5 py-1.5 text-xs font-black ${language === value ? "bg-white text-[#168fc7] shadow-sm" : "text-[#888]"}`}>{value === "zh" ? "中文" : value === "en" ? "EN" : "双语"}</button>)}
  </div>;
}
