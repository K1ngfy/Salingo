...
function AssistedText({ text }: { text: string }) {
  return <>{splitQuestionKeywords(text).map((part, index) => part.highlighted ? <mark key={`${part.value}-${index}`} className="rounded bg-[var(--c-fff0a8)] px-1 text-inherit">{part.value}</mark> : <span key={`${part.value}-${index}`}>{part.value}</span>)}</>;
}
...
export function QuestionStem({ question, language, className, assistEnabled = false }: { question: Question; language: ContentLanguage; className?: string; assistEnabled?: boolean }) {
  ...
  return <div className={className}>
    ...
    {content.secondary && <p className="mt-3 border-l-4 border-[var(--c-b9e5fa)] pl-4 font-semibold leading-7 text-[var(--c-667780)]">{assistEnabled ? <AssistedText text={content.secondary.stem} /> : content.secondary.stem}</p>}
    {warnings.length > 0 && <div className="mt-4 space-y-2 rounded-xl bg-[var(--c-fff8df)] p-3 text-sm font-bold text-[var(--c-8a681d)]">{warnings.map((warning) => <p key={warning}>{warning}</p>)}</div>}
  </div>;
}
...
export function QuestionAnswerInput({ question, response, onChange, language, disabled = false, reveal = false }: {
  ...
  if (question.type === "matching") {
    ...
    return <div className="mt-7 space-y-3">
      {prompts.map((prompt) => {
        ...
        return <label key={prompt.id} className={cn("block rounded-2xl border-2 bg-[var(--surface)] p-4", reveal && !wrong ? "border-[var(--c-58cc02)]" : wrong ? "border-[var(--c-ff4b4b)]" : "border-[var(--c-deded8)]") }>
          <span className="flex gap-3 font-bold"><strong className="text-[var(--c-168fc7)]">{prompt.id}.</strong><span>{prompt.text}{secondaryPrompts.get(prompt.id) && <span className="mt-1 block text-sm font-semibold text-[var(--c-7a858a)]">{secondaryPrompts.get(prompt.id)}</span>}</span></span>
          <select value={selected} disabled={disabled} onChange={(event) => onChange(matchingResponse({ ...matches, [prompt.id]: event.target.value }))} className="mt-3 h-12 w-full rounded-xl border-2 border-[var(--c-deded8)] bg-[var(--surface)] px-3 font-bold outline-none focus:border-[var(--c-1cb0f6)]">
          ...
          {reveal && wrong && <span className="mt-2 block text-sm font-black text-[var(--c-58a700)]">正确匹配：{correct}. {content.primary.options.find((option) => option.id === correct)?.text}</span>}
        </label>;
      })}
    </div>;
  }
  ...
  return <div className="mt-7 space-y-3">
    {content.primary.options.map((option) => {
      ...
      return <button key={option.id} type="button" onClick={() => choose(option.id)} disabled={disabled} className={cn("flex w-full items-center gap-4 rounded-2xl border-2 p-4 text-left font-bold transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky-100", state === "idle" && "border-[var(--c-deded8)] bg-[var(--surface)] shadow-[0_3px_0_var(--c-deded8)]", state === "selected" && "border-[var(--c-1cb0f6)] bg-[var(--c-e8f7ff)] text-[var(--c-137eae)] shadow-[0_3px_0_var(--c-1cb0f6)]", state === "correct" && "border-[var(--c-58cc02)] bg-[var(--c-eefbdc)] text-[var(--c-438c0d)]", state === "wrong" && "border-[var(--c-ff4b4b)] bg-[var(--c-fff0f0)] text-[var(--c-c53636)]") }>
